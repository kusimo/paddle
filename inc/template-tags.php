...
if( !function_exists( 'paddle_tag_lists' ) ) : 
	...
	function paddle_tag_list() {
		...
		if ( $tags_list ) {
			...
			printf( '<span class="tags-links" title="%2s">' . esc_html__( '%2s' ) . '</span>', esc_attr_x( 'Tagged', 'paddle' ), $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}	//	End paddle tag list
...
function paddle_excerpt_more( $link ) {
	...
	$link = sprintf( '<div class="article-more-link"><a href="%1$s" class="read-more">%2$s</a></div><div class="clearfix"></div>',
